...
CÓDIGO EN EL QUE TODAVÍA NO SE HA REVISADO LA OPCIÓN --exclusive-positions 
-> las posiciones solapadas en los resultados dentro de un mismo paciente se unifican al valor 
máximo que toma esa posición en el rango más pequeño (que por tanto le da un valor más alto).
Primando siempre el True sobre el False.
"""
...
def compute_recombination_profile(events: pd.DataFrame,
    ...
    if aggregate_by_patient and "sra" in ev.columns:
        ...
        ev_sra_groups = ev.groupby("sra", dropna=True)
...
def generate_recombination_html(
    ...
    MARGIN = dict(l=80, r=30, t=70, b=140)
    LEGEND_Y = -0.15
    X_TITLE_STANDOFF = 30
    Y_TITLE_STANDOFF = 15
    CAPTION_Y = -0.28

    caption_parts = ["Azul: ρ (recombination ratio)"]
    if begfin_conc_positions is not None and begfin_conc_millions is not None and max_begfin_conc_millions > 0:
        caption_parts.append("Rojo: concentración de inicios/finales (escalada)")
    if cov_positions is not None and cov_millions is not None and max_cov_millions > 0:
        caption_parts.append("Gris: profundidad de análisis (escalada)")
