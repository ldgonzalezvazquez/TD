...
def parse_clonalframeml_pair(
    ...
    for clade in tree.find_clades():
        label = clade.name
        if label is None or label.startswith("#"):   # (por si acaso)
            continue
...
def parse_rdp_csv(csv_path: Path, excluded: set[str]):
    """
    Procesa un archivo '*.fasta.csv' generado por RDP y devuelve:
        { (Begin, End): {
...
def append_df_to_excel(df: pd.DataFrame,
    ...
    if path.exists():
        ...
        with pd.ExcelWriter(path,
            ...
            # REGISTER existing sheets --> prevents pandas
            #    from trying to create 'events_by_tool1'
            wr.sheets = {ws.title: ws for ws in wr.book.worksheets}
    ...
    for ev_id, s_ev in santa.items():
        ...
        if cfml:
            for rk, (node, cd) in enumerate(cfml.items(), 1):
                ...
                # --- solapamiento de breakpoints (mejor de todas las alts) --
                # ------------------- breakpoints de CFML --------------------
                #  *Preferimos* los que vienen en det_par1/det_par2 (break_1/2);
                #  si no existen, caemos al viejo break_ini / break_end.
...
def integrate_rdp_results(tag: str,
    ...
    if not genome_len:                        # backup → RDP data
        genome_len = max(max(k) for k in rdp_events)
