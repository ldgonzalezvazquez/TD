...
import argparse, csv, re, pathlib, warnings, os, math, json, sys, locale
...
def _configure_locale_fallback():
    """Avoid locale crashes in HPC/conda environments."""
    candidates = []
    if os.environ.get("LC_ALL"):
        candidates.append(os.environ["LC_ALL"])
    if os.environ.get("LANG"):
        candidates.append(os.environ["LANG"])
    candidates.extend(["en_US.UTF-8", "C.UTF-8", "UTF-8", "C"])
...
def _normalize_patient_id_from_stem(stem: str) -> str:
    """
    Normaliza el identificador de paciente a partir del nombre base del TSV.
    Ejemplos:
      007-A_selected_ranked -> 007-A
      007-A_genes           -> 007-A
      007-A                 -> 007-A
    """
    pat = stem
    for suffix in ("_selected_ranked", "_genes"):
...
def make_long(tsv_path: pathlib.Path, depth_any=10, n_jobs=2, nperm=2000, freq_delta=0.01, thr_clear=0.95, eps_tie=0.05):
    ...
    pat = _normalize_patient_id_from_stem(tsv_path.stem)
...
def _merge_aa_changes_html_style(changes_list: List[str]) -> str:
    """
    Merge aa_change labels with the same overlap rule used in HTML tables.
    This keeps PDF labels aligned with the HTML view for multi-variant positions.
    """
    if not changes_list:
        return ""
    ...
    merged = valid[0]
    for curr in valid[1:]:
        m_nums = re.findall(r"(\d+)", merged)
        c_nums_start = re.findall(r"^(\d+)", curr)
        c_nums_end = re.findall(r"(\d+)$", curr)
        overlap = False

        if m_nums:
            num = m_nums[-1]
            if c_nums_start and num == c_nums_start[0]:
                if merged.endswith(num) or curr.startswith(num):
                    suffix = curr.split(num, 1)[1]
                    merged += suffix
                    overlap = True
            elif c_nums_end and num == c_nums_end[0]:
                if merged.startswith(num) or (num in merged):
                    prefix = curr.split(num, 1)[0]
                    if prefix and not merged.startswith(prefix):
    ...
    # Post-normalization to match displayed HTML convention in indel-amb mixed labels.
    # Example: "74Lamb(indel)" -> "74L(indel)"
    merged = re.sub(r"(\d+[A-Z\*])amb\(indel\)$", r"\1(indel)", merged)
    return merged
...
def summarise_positions(df_stats: pd.DataFrame) -> pd.DataFrame:
    ...
    for pos, sub in sel.groupby("pos"):
        ...
        aa_changes_all = [str(x).strip() for x in sub["aa_change"].tolist()
        ...
        if multiple_pos and not neg_sel.empty:
            ...
            # Alinear con la etiqueta fusionada mostrada en HTML
            merged_html = _merge_aa_changes_html_style(aa_changes_all)
            if merged_html:
                label = merged_html
            synonymous = None
            beta_up = p_up = beta_dn = p_dn = score_up = score_dn = None
            ...
            merged_html = _merge_aa_changes_html_style(aa_changes_all)
            label = merged_html if merged_html else (f"{aa_idx}amb" if aa_idx else "amb")
            synonymous = None
            beta_up = p_up = beta_dn = p_dn = score_up = score_dn = None
        elif multiple_neg and pos_sel.empty:
            # Solo múltiples bajadas → amb{aa_idx}
            merged_html = _merge_aa_changes_html_style(aa_changes_all)
            label = merged_html if merged_html else (f"amb{aa_idx}" if aa_idx else "amb")
            synonymous = None
            beta_up = p_up = beta_dn = p_dn = score_up = score_dn = None
        elif multiple_pos and multiple_neg:
            # Múltiples de ambos signos → amb{aa_idx}amb
            merged_html = _merge_aa_changes_html_style(aa_changes_all)
            label = merged_html if merged_html else (f"amb{aa_idx}amb" if aa_idx else "ambamb")
            synonymous = None
            beta_up = p_up = beta_dn = p_dn = score_up = score_dn = None
            ...
            # Alinear con etiqueta fusionada de HTML (negativo primero, luego positivo)
            neg_aa = _merge_aa_changes_html_style([str(x).strip() for x in neg_sel["aa_change"].tolist()])
            pos_aa = _merge_aa_changes_html_style([str(x).strip() for x in pos_sel["aa_change"].tolist()])
            merged_html = _merge_aa_changes_html_style([neg_aa, pos_aa]) if (neg_aa and pos_aa) else (pos_aa or neg_aa)
            if merged_html:
                label = merged_html
...
def plot_position(patient: str,
    ...
    # Título alineado con la lógica visible en HTML:
    # usar etiqueta resumida de posición cuando exista, y anclar siempre al nt real.
    if pos_row is not None and isinstance(pos_row, pd.Series):
        lbl = str(pos_row.get("label", "")).strip()
        if not lbl or lbl.lower() in {"nan", "none"}:
            pos_stats = df_stats[df_stats["pos"] == pos]
            aa_changes = [str(x).strip() for x in pos_stats.get("aa_change", pd.Series(dtype=str)).tolist()
                          if str(x).strip() and str(x).strip().lower() not in {"nan", "none"}]
            lbl = aa_changes[0] if aa_changes else "amb"
        ...
        ttl = f"{patient} · {lbl}/{pos} [{syn_txt}] (n_sel={n_sel})"
    else:
...
def _first_valid_per_pos(df: pd.DataFrame, value_col: str) -> dict:
    out = {}
    if value_col not in df.columns:
        return out
    for _, r in df[["pos", value_col]].dropna(subset=["pos"]).iterrows():
        try:
            p = int(r["pos"])
        except Exception:
            continue
...
def plot_genome(patient: str,
    ...
    # --- Usar preferentemente el *_genes.tsv para fijar el rango genómico completo ---
    # En replot-only, tsv_path suele ser *_selected_ranked.tsv y no contiene el rango completo.
    genes_candidates = [
        outdir / f"{patient}_genes.tsv",
        outdir.parent / f"{patient}_genes.tsv",
    ]
    if str(tsv_path.stem).endswith("_genes"):
        genes_candidates.append(tsv_path)
    genes_tsv = next((p for p in genes_candidates if p.is_file()), None)

    if genes_tsv is not None:
        try:
            genes_raw_all = pd.read_csv(genes_tsv, sep="\t")
            cols = set(genes_raw_all.columns)

            pos_col = None
            for cand in ("position", "pos", "nt_pos"):
                if cand in cols:
                    pos_col = cand
                    break

            gene_col = None
            for cand in ("gene", "product"):
                if cand in cols:
                    gene_col = cand
                    break

            if pos_col and gene_col:
                genes_raw = (genes_raw_all[[pos_col, gene_col]]
                             .rename(columns={pos_col: "position", gene_col: "gene"}))
            else:
                # Si el fichero no trae esas columnas, usar fallback seguro.
    ...
    genes_raw["position"] = pd.to_numeric(genes_raw["position"], errors="coerce")
    genes_raw = genes_raw.dropna(subset=["position"]).copy()
    ...
    if handles:
        legend = ax.legend(handles,
                           ...
                           ncol=max(1, len(gene_order)),  # evita ncol=0
                           fontsize=8,
                           title_fontsize=9,
                           frameon=True)
...
def process(tsv_path, outdir, nperm, n_jobs, resume, freq_delta,
    ...
    long_out = outdir/f"{pat}_long.tsv"
    if replot_only:
        print(f"\n🎨 Replot-only para paciente: {pat}")
        if not long_out.exists() or not stats_out.exists():
            print(f"❌ {pat}: faltan archivos para replot-only -> {long_out.name} o {stats_out.name}")
            return

        long = pd.read_csv(long_out, sep="\t")
        df_full = pd.read_csv(stats_out, sep="\t")
        if "pos" not in df_full.columns:
            print(f"❌ {pat}: {stats_out.name} no tiene columna 'pos'")
            return
        df_full["pos"] = pd.to_numeric(df_full["pos"], errors="coerce")
        df_full = df_full.dropna(subset=["pos"]).copy()
        df_full["pos"] = df_full["pos"].astype(int)
        if "beta" in df_full.columns:
            df_full["beta"] = pd.to_numeric(df_full["beta"], errors="coerce")

        gene_of = _first_valid_per_pos(df_full, "gene")
        aaidx_of = _first_valid_per_pos(df_full, "aa_idx")

        df = df_full.drop(columns=["day", "k", "n"], errors="ignore")
        beta_mean_all = df["beta"].mean() if "beta" in df.columns else None
        sel_mask = df["tag"].isin(["pos","neg"]) if "tag" in df.columns else pd.Series(False, index=df.index)
        beta_mean_sel = (df.loc[sel_mask, "beta"].mean()
                         if sel_mask.any() else None)

        print("📊 Regenerando resúmenes por posición...")
        pos_summary = summarise_positions(df)
        pos_summary.to_csv(outdir/f"{pat}_per_position_summary.tsv", sep="\t", index=False)
        pos2row = {r.pos: r for _, r in pos_summary.iterrows()}

        plot_dir = outdir/f"{pat}_plots"
        plot_dir.mkdir(exist_ok=True)
        print(f"🎨 Regenerando {len(pos_summary['pos'].unique())} gráficos de posición...")
        for pos in tqdm(pos_summary["pos"].unique(), desc="Replot posición", unit="pos"):
            plot_position(pat, int(pos), df, long, beta_mean_sel, beta_mean_all,
                          pos2row.get(pos), plot_dir, gene_of, aaidx_of)

        print("🌍 Regenerando gráfico genómico global...")
        plot_genome(pat, df, long, outdir, beta_mean_sel, beta_mean_all, tsv_path)
        print(f"✅ Replot-only completado para {pat}")
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--replot-only", action="store_true",
                        help="Rehace únicamente los PDFs (posXXXXX_ALL y genome_selection) usando *_long.tsv y *_variant_stats.tsv ya existentes")
