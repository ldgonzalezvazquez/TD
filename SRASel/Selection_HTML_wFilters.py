...
Plotly (htmls): Plotly Technologies Inc. Title: Collaborative data science. Publisher: Plotly Technologies Inc. Place of publication: Montréal, QC. Date of publication: 2015. URL: https://plot.ly"""
    ...
    if cache_file:
        ...
        # Asegurar que las claves de posición sean strings (como en el caché JSON)
        for patient in var_data:
            var_data[patient] = {str(pos): val for pos, val in var_data[patient].items()}
    ...
    def blob(i, o):
        return f'<script id="{i}" type="application/json">{json.dumps(o, cls=PlotlyJSONEncoder)}</script>'

    BLOBS = "".join(blob(k, v) for k, v in VIEWS.items())
    var_blob = (
        f'<script id="var-data" type="application/json">'
        f'{json.dumps(var_data, cls=PlotlyJSONEncoder)}'
        f'</script>'
    )
    selected_positions_blob = (
        f'<script id="selected-positions" type="application/json">'
        f'{json.dumps(selected_positions_data, cls=PlotlyJSONEncoder)}'
        f'</script>'
    )
    selected_positions_with_flags_blob = (
        f'<script id="selected-positions-flags" type="application/json">'
        f'{json.dumps(selected_positions_with_flags, cls=PlotlyJSONEncoder)}'
        f'</script>'
    )
    # Generar var_detailed para ambos modos
    var_detailed_gene_blob = (
        f'<script id="var-detailed-gene" type="application/json">'
        f'{json.dumps(VIEWS["gene"]["var_detailed"], cls=PlotlyJSONEncoder)}'
        f'</script>'
    )
    var_detailed_protein_blob = (
        f'<script id="var-detailed-protein" type="application/json">'
        f'{json.dumps(VIEWS["protein"]["var_detailed"], cls=PlotlyJSONEncoder)}'
        f'</script>'
    )
    ...
    mapping_blob = (
        ...
        f'{json.dumps(position_mapping, cls=PlotlyJSONEncoder)}'
        f'</script>'
    )
    ...
    filter_data_blob = (
        ...
        f'{json.dumps({"patient_data": patient_filter_data, "patient_rows": patient_filter_rows, "columns": filter_columns, "unique_values": filter_unique_values, "patient_name_mapping": patient_name_mapping}, cls=PlotlyJSONEncoder)}'
        f'</script>'
    )
    ...
    upgma_blob = (
        f'<script id="upgma-genes" type="application/json">{json.dumps(upgma_genes, cls=PlotlyJSONEncoder)}</script>'
        f'<script id="upgma-proteins" type="application/json">{json.dumps(upgma_proteins, cls=PlotlyJSONEncoder)}</script>'
    )
    ...
    segments_blob = f'<script id="segments" type="application/json">{json.dumps(segments, cls=PlotlyJSONEncoder)}</script>'
    ...
    JS = """
         ...
         const DATA = {
           gene    : JSON.parse(document.getElementById('gene').textContent),
           protein : JSON.parse(document.getElementById('protein').textContent)
         };
        let mode = 'gene';
        ...
        const varData    = JSON.parse(document.getElementById('var-data').textContent);
         const positionMapping = JSON.parse(document.getElementById('position-mapping').textContent);
         const UPGMA_GENES = JSON.parse(document.getElementById('upgma-genes').textContent);
         const UPGMA_PROT  = JSON.parse(document.getElementById('upgma-proteins').textContent);
         const SEGMENTS    = JSON.parse(document.getElementById('segments').textContent);
        const selectedPositionsData = JSON.parse(document.getElementById('selected-positions').textContent);
        const SELECTED_POSITIONS = selectedPositionsData; // Alias para compatibilidad
        // Cargar datos detallados para ambos modos
        const VAR_DETAILED_GENE = JSON.parse(document.getElementById('var-detailed-gene').textContent);
        const VAR_DETAILED_PROTEIN = JSON.parse(document.getElementById('var-detailed-protein').textContent);
        
        // Función para obtener los datos correctos según el modo
        function getVarDetailed() {
          return mode === 'gene' ? VAR_DETAILED_GENE : VAR_DETAILED_PROTEIN;
        }
        ...
        function popupGenePlot(cat, x, det, traceName, pointIndex, container){
          ...
          // Construir título usando directamente el nombre de la subdivisión
          let title;
          console.log('DEBUG title construction:', {x, fullName, cat});
          
          // Usar directamente el nombre de la subdivisión (x) que ya contiene la información correcta
          if (x.includes(' - ') && x.includes('(') && x.includes(')')) {
            // x tiene el formato: "ORF1ab - 17137 (5625)" o "484E - 23012 (484)"
            // Extraer la parte después del primer " - " para evitar duplicar el gen
            const parts = x.split(' - ');
            if (parts.length >= 2) {
              const positionPart = parts.slice(1).join(' - '); // "17137 (5625)" o "23012 (484)"
              // Convertir formato (5625) a / S5625
              const positionMatch = positionPart.match(/^(\d+)\s*\((\d+)\)$/);
              if (positionMatch) {
                const ntPos = positionMatch[1];
                const aaPos = positionMatch[2];
                // Buscar la notación aminoacídica en fullName
                let aaNotation = '';
                if (fullName && fullName.includes(' / ')) {
                  const aaPart = fullName.split(' / ')[0];
                  aaNotation = aaPart; // ej: "S5625", "Q3729K", etc.
                } else {
                  aaNotation = `S${aaPos}`; // fallback
                }
                title = `${cat} - ${ntPos} / ${aaNotation}`;
                console.log('DEBUG using x with amino acid notation:', title);
              } else {
                title = `${cat} - ${positionPart}`;
                console.log('DEBUG using x without duplication:', title);
              }
            } else {
              title = `${cat} - ${x}`;
              console.log('DEBUG using x directly:', title);
            }
          } else if (fullName && fullName.includes(' / ')) {
            // Si no tiene el formato esperado, usar fullName
            const aaPart = fullName.split(' / ')[0];
            const ntPart = fullName.split(' / ')[1];
            title = `${cat} - ${aaPart} / ${ntPart}`;
            console.log('DEBUG using fullName:', title);
            } else {
            title = `${cat} - ${x}`;
            console.log('DEBUG fallback title:', title);
          }
          ...
          if (Object.keys(det).length > 0) {
            ...
            Object.keys(det).sort().forEach(p => {
              ...
              const pos = fullName.split(' / ')[1]; // Extraer posición nucleotídica
              const plotPath = `${dataDir}/${p}_plots/pos${pos.padStart(5, '0')}_ALL.pdf`;
        ...
        function formatCovaryingWith(covaryingWithStr) {
          if (!covaryingWithStr || covaryingWithStr.trim() === '') {
            return 'No covariation data available';
          }
          ...
          return formattedItems.length > 0 ? formattedItems.join('') : 'No covariation data available';
        }
        ...
        function drawGeneBars(){
          ...
          // Build filtered category list to avoid gaps on X
          const filteredCategoryOrder = [];
          const seenCategories = new Set();
          filteredTraces.forEach(trace => {
            (trace.x || []).forEach(xVal => {
              if (!seenCategories.has(xVal)) {
                seenCategories.add(xVal);
                filteredCategoryOrder.push(xVal);
              }
            });
          });
        ...
        function updateHeatmap1View() {
          ...
          try {
            ...
            if (selectedPatients.length > 0 || selectedItems.length > 0) {
              ...
              Plotly.newPlot(varFig, filteredData, varHeatLayout)
                .then(() => {
        ...
        function updateHeatmap2View() {
          ...
          try {
            ...
            if (selectedPatients.length > 0 || selectedItems.length > 0) {
              ...
              Plotly.newPlot(fig, filteredData, layout)
                .then(() => {
        ...
        toggle.onclick = () => {
          mode = (mode === 'gene') ? 'protein' : 'gene';
          toggle.textContent = (mode === 'gene') ? '🔄 Protein View' : '🔄 Gene View';
          updateSelectors();
          // Actualizar automáticamente todo al cambiar de vista
          applyFilters();
          // Redibujar UPGMA acorde a la vista, y limpiar el otro
          try {
            if (mode === 'gene') {
              if (document.getElementById('upgma-genes-div')) drawUPGMA();
              const other = document.getElementById('upgma-proteins-div');
              if (other) other.innerHTML = '';
            } else {
              if (document.getElementById('upgma-proteins-div')) drawUPGMA();
              const other = document.getElementById('upgma-genes-div');
              if (other) other.innerHTML = '';
            }
        ...
        window.__do_initial_render__ = () => {
          ...
          try { drawGeneBars(); } catch(e) { console.error('base gene bars error', e); }
          try { drawNspBars(); } catch(e) { console.error('base nsp bars error', e); }
        ...
        function drawVarPlot(){
          ...
          varPlotDiv.innerHTML = "";
            ...
            const segs = (mode === 'gene') ? SEGMENTS.genes : SEGMENTS.proteins;
